...
public class ItemRestBean {
    ...
    @POST
    @Path("like/{itemId}")
    public Response likeComment(@PathParam("itemId") int itemId) throws Exception {
	if (itemId < 1) {
	    return Response.status(Status.NOT_ACCEPTABLE).build();
	}

	Item item = itemService.retrieveItemCreateIfNull(itemId);
	return like(item, true);
    }

    @DELETE
    @Path("like/{itemId}")
    public Response dislikeComment(@PathParam("itemId") int itemId) throws Exception {
