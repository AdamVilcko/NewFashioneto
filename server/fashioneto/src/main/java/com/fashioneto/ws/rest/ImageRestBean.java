...
public class ImageRestBean {
    ...
    @GET
    @Path("raw/{imageId}")
    public Response getImageStandardSize(@PathParam("imageId") int imageId) throws IOException {
	return getResponseImageContent(imageId, ImageSize.STANDARD);
    }

    @GET
    @Path("raw/{imageSize}/{imageId}")
    public Response getImageStandardSize(@PathParam("imageSize") ImageSize imageSize, @PathParam("imageId") int imageId)
	    throws IOException {
    ...
    @POST
    @Path("like/{imageId}")
    public Response likeComment(@PathParam("imageId") int imageId) throws NoUserInContextException {
	if (imageId < 1) {
    ...
    @DELETE
    @Path("like/{imageId}")
    public Response dislikeComment(@PathParam("imageId") int imageId) throws NoUserInContextException {
	Image image = imageService.getImage(imageId);
